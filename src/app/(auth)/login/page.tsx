...
export default function LoginPage() {
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-lg border border-white/10 bg-white/[0.04] p-7 shadow-[0_1px_2px_rgba(0,0,0,0.2),0_30px_60px_rgba(0,0,0,0.35)] backdrop-blur">
      <h2 className="font-display font-semibold text-[20px] text-white">Sign in</h2>
      <p className="mt-1 text-[13px] text-white/45">Enter your credentials to access the platform.</p>

      <button
        type="button"
        onClick={() => doSignIn(DEMO_EMAIL, DEMO_PASSWORD)}
        disabled={loading}
        className="mt-5 flex w-full items-center justify-between rounded-md border border-[#22c9bc]/35 bg-[#22c9bc]/8 px-3 py-2.5 text-left transition-all hover:border-[#22c9bc]/55 hover:bg-[#22c9bc]/14 active:scale-[0.99] disabled:opacity-50"
      >
        <span>
          <span className="block text-[13px] font-semibold text-[#5eead4]">Try the demo account</span>
          <span className="mt-0.5 block text-[11px] text-white/45">{DEMO_EMAIL} · pre-loaded with sample trades</span>
        </span>
