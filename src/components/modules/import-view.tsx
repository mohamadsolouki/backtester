...
export function ImportView({ batches }: { batches: SerializedBatch[] }) {
  ...
  function handleFile(file: File) {
    ...
    reader.onload = (e) => {
      ...
      const parsed = parseMetaTraderFile(content);
      setResult(parsed);
  ...
  return (
    ...
    >
      <Surface>
        ...
        <div className="mt-3">
          <label className="flex min-h-32 cursor-pointer flex-col items-center justify-center rounded-md border-2 border-dashed border-[#cfd8d4] p-6 text-center hover:border-[#0f9f95] hover:bg-[#effaf8]/50">
            <Upload className="h-8 w-8 text-[#0f9f95]" />
            <div className="mt-2 text-[14px] font-semibold">Drop file or click to browse</div>
            <p className="mt-1 text-[12px] text-[#66746f]">
              Supports MT4 HTML statements, MT5 XML exports, and CSV files
            </p>
            <input
              type="file"
              className="hidden"
              accept=".html,.htm,.xml,.csv,.txt"
              onChange={(e) => {
        ...
        <Surface>
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button onClick={toggleAll} className="h-8 rounded-md border border-[#cfd8d4] px-3 text-[12px] font-medium hover:bg-[#f5f7f4]">
                {selected.size === result.trades.length ? "Deselect All" : "Select All"}
              </button>
              <button
                onClick={importSelected}
                disabled={importing || selected.size === 0}
                className="h-8 rounded-md bg-[#0f9f95] px-4 text-[12px] font-semibold text-white hover:bg-[#08746f] disabled:opacity-50"
              >
          ...
          {result.errors.length > 0 && (
            <div className="mt-3 rounded-md border border-amber-200 bg-amber-50 p-3">
              ...
              <div className="mt-2 space-y-1 text-[12px] text-amber-600">
                ...
                {result.trades.map((t, i) => (
                  <tr
                    ...
                    className={cn(
                      ...
                      selected.has(i) && "bg-[#effaf8]"
                    )}
                  >
                    <td className="px-2">
                      <span className={cn(
                        "flex h-4 w-4 items-center justify-center rounded border",
                        selected.has(i) ? "border-[#0f9f95] bg-[#0f9f95] text-white" : "border-[#98a5a0]"
                      )}>
                    ...
                    <td className={cn("px-2 font-semibold", t.profit >= 0 ? "text-[#08746f]" : "text-[#d94848]")}>
        ...
        <Surface>
          ...
          <div className="mt-3 overflow-x-auto">
            <table className="w-full text-left text-[12px]">
              ...
              <tbody>
                {batches.map((b) => (
                  <tr key={b.id} className="border-b border-[#edf1ef]">
                    ...
                    <td className="px-2 text-[#08746f]">{b.validRows}</td>
                    <td className="px-2">{b.errorRows > 0 ? <span className="text-[#d94848]">{b.errorRows}</span> : "0"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
